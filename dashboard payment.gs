...
// เช่น "น้องชานนท์ 250x14=3,500\nน้องณิชา 150×8=1,200"
// สำหรับ onsiteDay: "01/12/68 น้องชานนท์(3.0) น้องณิชา(2.5)"
// ============================================================
function createSummaryDetailsText(studentDetails, courseType = null) {
  const lines = [];

  // ถ้าเป็น onsiteDay ให้จัดกลุ่มตามวันที่
  if (courseType === 'onsiteDay') {
  ...
  // สำหรับ courseType อื่นๆ - แสดงเฉพาะที่จ่ายแล้ว
  const paidStudents = [];
...
// แสดงรูปแบบ: วันที่ + รายชื่อนักเรียน(ชม.)
// สำหรับ onsiteDay: คิดเงินเหมาวัน ดังนั้นนับ amount ครั้งเดียวต่อวัน (850 บาท/วัน)
// ============================================================
function groupOnsiteDayByDate(students) {
    ...
    studentData.sessions.forEach(session => {
      ...
      if (!dateGroups.has(dateKey)) {
        dateGroups.set(dateKey, {
          ...
          totalHours: 0,
          totalAmount: 0,
          isPaid: false  // เก็บสถานะว่าวันนี้จ่ายแล้วหรือยัง
        });
      }

      const dateGroup = dateGroups.get(dateKey);

      // เช็คว่านักเรียนคนนี้มีในวันนี้แล้วหรือยัง
      const existingStudent = dateGroup.students.find(s => s.name === studentData.studentName);
      if (!existingStudent) {
        dateGroup.students.push({
          name: studentData.studentName,
          hours: session.duration || 0,
          amount: studentData.amount || 0
        });
        dateGroup.totalHours += session.duration || 0;

        // เก็บสถานะการจ่ายเงิน (ถ้ามีนักเรียนคนใดคนหนึ่งจ่ายแล้ว = วันนี้จ่าย)
        if (studentData.status === '✅ จ่าย') {
          dateGroup.isPaid = true;
        }
      } else {
        // ถ้ามีแล้ว ให้บวกชั่วโมงเพิ่ม (กรณีมีหลาย session ในวันเดียวกัน)
        existingStudent.hours += session.duration || 0;
        dateGroup.totalHours += session.duration || 0;
  ...
  // คำนวณ totalAmount ต่อวัน (เหมาวันละ 850 บาท)
  dateGroups.forEach((dateGroup, dateKey) => {
    if (dateGroup.isPaid) {
      // onsiteDay เป็นเหมาวัน ดังนั้นคิด 850 บาทต่อวัน ไม่ว่าจะมีกี่คน
      dateGroup.totalAmount = 850;
    }
  });

  // Debug: แสดงวันทั้งหมดที่จัดกลุ่มได้
  Logger.log(`📅 groupOnsiteDayByDate found ${dateGroups.size} days: ${Array.from(dateGroups.keys()).join(', ')}`);

  // เรียงตามวันที่ (ล่าสุดก่อน)
  const sorted = new Map([...dateGroups.entries()].sort((a, b) => {
    const dateA = parseDateValue(a[1].date);
    const dateB = parseDateValue(b[1].date);
